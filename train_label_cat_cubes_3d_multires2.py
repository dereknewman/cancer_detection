...
local4 = pool2_flatten
weights = _variable_with_weight_decay('weights7', [520, NUM_CLASSES],
                                      stddev=1/520.0, wd=0.0)
...
f_train = open("train6_mres2_rand_" + str(lr) + ".txt","a")
f_test = open("test6_mres2_rand_" + str(lr) + ".txt","a")
...
for index in range(10000):
    ...
    train_results, labels_1 = sess.run([accuracy,label_onehot_],feed_dict={transpose_index: [0,1,2], k_value: 0})
    label_sum += np.sum(labels_1, axis=0)
    print(train_results," -> ", label_sum)
    ...
    f_test.flush()
    if np.mod(index,9)==0:
        save_path = saver.save(sess, "/media/derek/disk1/kaggle_ndsb2017/saved_models_mres2/model.ckpt")
