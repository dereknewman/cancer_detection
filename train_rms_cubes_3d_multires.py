...
################## SECOND TRAIN HEAD #####################################
##########################################################################
weights = _variable_with_weight_decay('weights7_head2', [192, 6],
                                      stddev=1/192.0, wd=0.0)
biases = _variable_initializer('biases7_head2', [6],
                          tf.constant_initializer(0.0))
softmax_linear = tf.add(tf.matmul(local4, weights), biases, name='scope.name7_head2')
cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(
        labels=mal, logits=softmax_linear, name='cross_entropy_per_example')
...
f_train = open("testACC_rms_mres1_rand_" + str(lr) + ".txt","a")
f_test = open("test_rms_mres1_rand_" + str(lr) + ".txt","a")
f_acc = open("train_rms_mres1_rand_ACC_" + str(lr) + ".txt","a")
...
for index in range(10000):
    ...
    f_acc.write(str(test_acc) + "\n")
    f_train.flush()
    f_test.flush()
    f_acc.flush()
    if np.mod(index,9)==0:
        save_path = saver.save(sess, "/media/derek/disk1/kaggle_ndsb2017/saved_models_rms_mres1/model.ckpt")
