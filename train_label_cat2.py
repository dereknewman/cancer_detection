...
dim = 4096
weights1_flat = _variable_with_weight_decay('weights3', shape=[dim, 384],
                                      stddev=0.04, wd=0.004)
biases1_flat = _variable_initializer('biases3', [384], tf.constant_initializer(0.1))
activation1_flat = tf.nn.relu(tf.matmul(pool2_flat, weights1_flat) + biases1_flat, name='scope.name3')
_activation_summary(activation1_flat)

# local4
#with tf.variable_scope('local4') as scope:
weights2_flat = _variable_with_weight_decay('weights4', shape=[384, 192],
                                      stddev=0.04, wd=0.004)
biases2_flat = _variable_initializer('biases4', [192], tf.constant_initializer(0.1))
activation2_flat = tf.nn.relu(tf.matmul(activation1_flat, weights2_flat) + biases2_flat, name='scope.name4')
_activation_summary(activation2_flat)
...
weights3_flat = _variable_with_weight_decay('weights5', [192, NUM_CLASSES],
                                      stddev=1/192.0, wd=0.0)
biases3_flat = _variable_initializer('biases5', [NUM_CLASSES],
                          tf.constant_initializer(0.0))
softmax_linear = tf.add(tf.matmul(activation2_flat, weights3_flat), biases3_flat, name='scope.name')
